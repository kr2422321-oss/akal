...
public:
    ...
    void printUsage() const
    {
        ...
        std::wcout << L"Usage:\n"
                   << L"  chrome_inject.exe [options] <chrome|brave|edge|opera|opera_gx|all>\n\n"
                   << L"Options:\n"
                   ...
                   << L"  opera     - Extract from Opera\n"
                   << L"  opera_gx  - Extract from Opera GX\n"
...
public:
    ...
    std::vector<std::pair<std::wstring, std::wstring>> findAllInstalledBrowsers()
    {
        ...
        for (const auto &[browserType, exeName] : supportedBrowsers)
        {
            ...
            // If Opera detected but path flavor mismatches, try preferred folders
            if (!path.empty() && (browserType == L"opera" || browserType == L"opera_gx"))
            {
                std::wstring pathLower = path; std::transform(pathLower.begin(), pathLower.end(), pathLower.begin(), ::towlower);
                bool isGx = pathLower.find(L"opera gx") != std::wstring::npos;
                if ((browserType == L"opera" && isGx) || (browserType == L"opera_gx" && !isGx))
                {
                    wchar_t localAppDataPath[MAX_PATH] = {0};
                    if (SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, localAppDataPath)))
                    {
                        fs::path candidate = fs::path(localAppDataPath) / "Programs" / (browserType == L"opera_gx" ? L"Opera GX" : L"Opera") / L"opera.exe";
                        if (fs::exists(candidate))
                            path = candidate.wstring();
                    }
                }
            }
            // If Opera not found in registry at all, probe known install location
            if (path.empty() && (browserType == L"opera" || browserType == L"opera_gx"))
            {
                wchar_t localAppDataPath[MAX_PATH] = {0};
                if (SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, localAppDataPath)))
                {
                    ...
                    {
                        path = candidate.wstring();
                        m_console.Debug(std::string("Resolved Opera path via default install location: ") + Utils::WStringToUtf8(path));
                    }
                }
            }
            if (!path.empty())
    ...
    static std::optional<Configuration> CreateFromArgs(int argc, wchar_t *argv[], const Console &console)
    {
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            if (arg == L"--verbose" || arg == L"-v")
                config.verbose = true;
            else if (arg == L"--no-kill")
                config.noKill = true;
            else if (arg == L"--no-network-kill")
                config.noNetworkKill = true;
        ...
        // Prefer known install locations for Opera flavors when registry is ambiguous
        if ((config.browserType == L"opera" || config.browserType == L"opera_gx"))
        {
            auto pathUtf8 = Utils::WStringToUtf8(config.browserDefaultExePath);
            wchar_t localAppDataPath[MAX_PATH] = {0};
            if (config.browserDefaultExePath.empty() || (config.browserType == L"opera" && pathUtf8.find("Opera GX") != std::string::npos) || (config.browserType == L"opera_gx" && pathUtf8.find("Opera GX") == std::string::npos))
            {
                if (SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_LOCAL_APPDATA, nullptr, 0, localAppDataPath)))
                {
                    fs::path candidate = fs::path(localAppDataPath) / "Programs" / (config.browserType == L"opera_gx" ? L"Opera GX" : L"Opera") / L"opera.exe";
                    if (fs::exists(candidate))
                        config.browserDefaultExePath = candidate;
                    console.Debug("Adjusted Opera path to: " + Utils::WStringToUtf8(candidate.wstring()));
                }
            }
        }
...
void ProcessAllBrowsers(const Console &console, bool verbose, const fs::path &outputPath)
{
    ...
    for (size_t i = 0; i < installedBrowsers.size(); ++i)
    {
        ...
        static const std::map<std::wstring, std::pair<std::wstring, std::string>> browserMap = {
            ...
            {L"opera_gx", {L"opera.exe", "Opera GX"}}};
