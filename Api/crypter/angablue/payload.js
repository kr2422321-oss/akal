...
const hideSelf = async (shouldHide) => {
    if (shouldHide === false) {
        silentLog('hideSelf function is not executed as shouldHide is false.');
        return;
    }
  ...
  `;
    ...
    const command = `powershell.exe -NoProfile -Command "${powershellScript.replace(/(\r\n|\n|\r)/gm, ' ').replace(/"/g, '\\"')}"`;
    try {
        execSync(command, { stdio: 'inherit' });
        silentLog('CMD window hidden successfully');
    } catch (error) {
        silentLog(`Error executing PowerShell script: ${error.message}`, true);
    }
};
